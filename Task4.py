...
if __name__ == '__main__':
    ...
    msg_recievers = set([data[1] for data in texts])
    
    call_senders_susp = []
    
    for call_sender in call_senders:
        if (call_sender not in call_receivers and call_sender not in msg_senders and call_sender not in msg_recievers):
            call_senders_susp.append(call_sender)
    call_senders_susp.sort()
    
    print("\n These numbers could be telemarketers: ")
    for numbers in call_senders_susp:
        print(numbers)
